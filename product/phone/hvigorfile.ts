...
//         const signTargetName = targetServices.getTargetData().getTargetName();
//         // 构建的未签名的hap的输出根目录
//         const M_MODULE_BUILD_OUTPUT_DIE: string = path.resolve(PROJECT_ROOT_PATH, 'product', 'phone', `build/default/outputs/${signTargetName}/`);
//
//         // 未签名的hap包路径
//         const INPUT_FILE: string = path.resolve(M_MODULE_BUILD_OUTPUT_DIE, `${M_MODULE_NAME}${ENTRY_NAME ? '-' + ENTRY_NAME : ''}-${signTargetName}-unsigned.hap`);
//         // 签名后的hap包路径
//         const OUTPUT_FILE: string = path.resolve(M_MODULE_BUILD_OUTPUT_DIE, `${M_MODULE_NAME}${ENTRY_NAME ? '-' + ENTRY_NAME : ''}-${signTargetName}-signed.hap`);
//
//         executeOnlineSign(INPUT_FILE, OUTPUT_FILE);
//
//     }, `${ONLINE_SIGNHAP_TASK_NAME}_${curTargetName}`).dependsOn(`${curTargetName}@PackageHap`);
//
//     // 使用在线签名,可以把离线签名任务disable掉
//     if (ONLINE_SIGN_TASK.getEnabled()) {
//         M_MODULE.getTaskByName(`${curTargetName}@SignHap`).setEnabled(false);
//     }
//     M_MODULE.getTaskByName('assembleHap').dependsOn(`${ONLINE_SIGNHAP_TASK_NAME}_${curTargetName}`);
// });
// const config = {
//     hvigor:hvigor,
//     packageConfig : {
//         appName: 'IntelligentScene', // 与cde架构定义的模块名相同，每个模块流水线归档的hap名，不填默认是module.json5定的模块名
